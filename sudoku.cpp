...
int main(int argc, char** argv)
{
    ...
    //范围限制
    if(full_num && game_num && (file_name!=nullptr))
        throw("输入异常！请只要求生成终盘或游戏或解数独！\n");
    else if (!game_num && (if_u||level||dig_up!=-1))
        throw("输入异常！生成游戏时请设置-c参数！\n");
    if (dig_up == -1 && dig_down == -1) {
        dig_up = 55;
        dig_down = 20;
    }
    if (dig_up > 55 || dig_down < 20 )
        throw("输入异常！生成游戏时的挖空个数必须在20~55之间！\n");
    char arg1 = argv[1][1];
    int num = argc;
    if (full_num) {
        ...
        if (game_num && level && !if_u) {
            ...
            while (1) {
                ...
                case 3:
                    ...
                    break;
                case 2:
                    if (count_simple <15 || endtime * 1000 > 40 && endtime * 1000 < 900)
                        save = true;
                    break;
                }
                if (save) {
    ...
    //char arg1 = argv[1][1];           //获取参数1
    //int arg2 = getnumber(argv[2]);    //获取参数2

    //if (arg1 == 'c') {                //-c
    //    //int x = getnumber(argv[2]);
    //    generate_c(arg2);             //生成arg2个数独终盘
    //}
    //if (arg1 == 'n') {
    //    
    //    /*if ('0' < argv[2][0] < '9') {
