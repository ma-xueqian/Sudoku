...
void generate_c(int num) {
	...
	char formatline[19] = { '1',' ','2',' ','3',' ','4',' ','5',' ','6',' ','7',' ','8',' ','9','\n','\0' };
...
//void generategame(int num, int num1=-1,int num2=-1) {
...
void getdig(char* str, int& digup, int& digdown)
{
	int res = 0;
	const char* p;
	for (p = str; *p; p++) {
...
bool dfs(int start, int& ifsolve)//从０开始依次遍历81个格子，计算此数独,ifsolve用于计数一共有多少个解
{
	...
	else
	{
		...
		if (board[row][col] == '$')
		{
			for (int i = 1; i <= 9; i++)
			{
				/*if (people_check(row, col)) {
					count_simple++;
				}*/
				if (is_valid(row, col, i))//从１－９依次放入空格，并判断是否合法
				{
					board[row][col] = i + '0';//如果有数字合法，就写入该数字的字符
					ok = dfs(start + 1, ifsolve);//判断此方格的下一个方格是否成功写入
...
int test_main(int myac, char** myav)
{
	int full_num = 0;
	...
	int dig_down = -1;
	bool if_u = false;
	for (int i = 1; i < myac; i++) {
		if (myav[i][1] != 'u' && i + 1 >= myac) {
			throw std::exception("命令行参数个数异常");
		}
		if (myav[i][1] == 'c') {
			full_num = getnumber(myav[i + 1]);
			i++;
		}
		else if (myav[i][1] == 's') {
			file_name = myav[i + 1];
			...
			game_num = getnumber(myav[i + 1]);
			i++;
		}
		else if (myav[i][1] == 'm') {
			level = getnumber(myav[i + 1]);
			i++;
		}
		else if (myav[i][1] == 'r') {
			getdig(myav[i + 1], dig_up, dig_down);
			i++;
		}
		else if (myav[i][1] == 'u') {
			if_u = true;
	...
	//范围限制
	if ((full_num && game_num) || (full_num && (file_name != nullptr)) || (game_num && (file_name != nullptr)))
	...
	if (dig_up > 55 || dig_down < 20)
		throw std::exception("输入异常！生成游戏时的挖空个数必须在20~55之间！\n");
	char arg1 = myav[1][1];
	int num = myac;
	if (full_num) {
		int arg2 = getnumber(myav[2]);
		generate_c(arg2);
	}
	if (file_name) {
		std::fstream outfile;
	...
	if (game_num && level && !if_u) {
		...
		while (1) {
			...
			case 3:
				...
				break;
			case 2:
				if (count_simple < 15 || endtime * 1000 > 40 && endtime * 1000 < 500)
					save = true;
				break;
			}
			if (save) {
	...
	if (game_num && level && if_u) {
		...
		while (1) {
			...
			if (save && ifsolve == 1 && game_num) {
				...
				infile.open("question.txt", std::ios::in);
				std::fstream outfile;
				outfile.open("one_answer_level_question.txt", std::ios::app);
				char temp;
				int j = 0;
				while (j < 9) {
					for (int i = 0; i < SIZE; i++) {
